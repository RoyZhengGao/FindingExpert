...
public class MyRetrievalModel {
	...
	public Map<String,Double> search(String query)
		...
		for (Map.Entry entry : sortedMap.entrySet()) {
			if(result.containsKey(ixreader.getNodeId((Integer) entry.getKey()))){
				result.put(ixreader.getNodeId((Integer) entry.getKey()), result.get(ixreader.getNodeId((Integer) entry.getKey()))+(-1)/(Math.log10((Double) entry.getValue())));
			}else
				result.put(ixreader.getNodeId((Integer) entry.getKey()), (-1)/(Math.log10((Double) entry.getValue())));
		}
		return result;
	}
